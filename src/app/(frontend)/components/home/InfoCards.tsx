    ...
    <section className="bg-[#F6F4EC] py-40 px-6">
      <div className="max-w-[93rem] mx-auto flex flex-wrap justify-center gap-10">
        {cardData.map((card, i) => (
          ...
          <div
            key={i}
            className={`flex flex-col justify-start bg-[#EEEADE] md:w-[30%] min-h-[680px] rounded-[1rem] p-10 text-center text-[#264C84] ${schibstedGrotesk.className} space-y-6 shadow-sm`}
          >
            {/*card image placeholder*/}
            <p className="text-base p-10 pt-25">(illustration)</p>

            {/*card title*/}
            <h2 className={`text-5xl ${fraunces.className}`}>{card.title}</h2>
            ...
            <div className="space-y-3">
              ...
              {card.contacts && (
                ...
                <ul
                  className={`text-base flex flex-col items-center font-semibold p-5 ${schibstedGrotesk.className}`}
                >
